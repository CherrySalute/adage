...
//랜덤 방식으로 0,1,2,3중에서 2개를 가져온다. (조건, 동일한 숫자가 존재하지 않음)
var nike_num_arr = [];
var nike_number;

while(nike_num_arr.length < 2){
    nike_number = Math.floor(Math.random() * 4); // 0 ~ 3
    // 최소값 0.0001 -> (*4) -> 0.0004 -> 내림 0
    // 최대값 0.9999 -> (*4) -> 3.9996 -> 내림 3
    if(nike_num_arr.indexOf(nike_number) == -1){ //배열 데이터 값이 일치하는것이 없다면
...
while(adidas_num_arr.length < 2){
    ...
    // 최소값 0.0001 -> (*4) -> 0.0004 -> 내림 0
    // 최대값 0.9999 -> (*4) -> 3.9996 -> 내림 3
    if(adidas_num_arr.indexOf(adidas_number) == -1){ //배열 데이터 값이 일치하는것이 없다면
...
while(puma_num_arr.length < 2){
    ...
    // 최소값 0.0001 -> (*4) -> 0.0004 -> 내림 0
    // 최대값 0.9999 -> (*4) -> 3.9996 -> 내림 3
    if(puma_num_arr.indexOf(puma_number) == -1){ //배열 데이터 값이 일치하는것이 없다면
