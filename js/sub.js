...
var $hash = location.hash; // url 로 부터 #(hash)을 포함한 우측의 문구 가져오기
...
var $contCover = document.querySelector("#cont .wrap .cont"); // 내부 이미지 공간
...
var hash_apply = function(){
    ...
    if($hash_txt){ // hash라는 정보가 존재한다면
        ...
        for(i=0;i<4;i++){
            ...
            // => 각 아이템마다 클릭햇을 때 detail.html로 이동하는 과정에서 던져줄 정보값(예시, nike_3)을 함께 보낸다.
        }
        $contCover.innerHTML = $contGroup;
    }
    else{ // hash라는 정보가 존재하지 않다면 -> 메인 페이지로 이동 시킨다.
        location.href = "./"; // index.html로 이동시킨다.
    }
}
...
for(const v of $list){ // const 예약어 사용시, 요소의 위치를 확실하게 고정시킴
    console.log(v);
    v.addEventListener("click", function(){ // 상단 메뉴 중에서 사용자가 어딘가를 클릭했을 때,
        ...
        console.log($hash_txt); // 클릭한 li 태그에 담긴 rel 속성값
